...
def fast():
    ...
    if not Path(model_path + 'head').exists() or not Path(model_path + 'body').exists():
        ...
        for headline, body_id, stance in tqdm(stances):
            ...
            #clean_text = [i + j for i, j in zip(clean_headline, clean_body)]
            clean_text = clean_headline + " " + clean_body
            clean_text = get_tokenized_lemmas(clean_text)
            
            
            clean_headline = get_tokenized_lemmas(clean_headline)
            clean_body = get_tokenized_lemmas(clean_body) #get_sent_tokenized_lemmas
        ...
        print(f"Training embedding model on All Text")
        model_y = FastText(sentences=clean_texts, size=size, iter=epochs, sg=1, workers=workers, min_count = 1)
        print("Saving")
        model_y.save(model_path + 'text')
        print("Saving done")
        ...
        model_y = FastText.load(model_path + 'text')
        print("loading done")
    return model_h, model_b, model_y
...
def embedding_features(headlines, bodies):
    ...
    for i, (headline, body) in tqdm(enumerate(zip(headlines, bodies))):
        ...
        #clean_all = [q + j for q, j in zip(clean_headline, clean_body)]

        clean_all = clean_headline + " " + clean_body
    
        clean_all = " ".join(get_tokenized_lemmas(clean_all))
        
        clean_headline = " ".join(get_tokenized_lemmas(clean_headline))
        body_s = " ".join(get_tokenized_lemmas(clean_body)) #get_sent_tokenized_lemmas
        
        clean_headlines.append(clean_headline)
        clean_bodies.append(body_s)
        
        #clean_all = [q + j for q, j in zip(clean_headline, body_s)]
        clean_alls.append(clean_all)#clean_all
       
    N = []
    model_h, model_b, model_y = fast()
    for i, (clean_t) in tqdm(enumerate(clean_alls)):
            j = np.zeros(300)
            jy = [float(x) for x in j.tolist()]
            try:
                y = [float(x) for x in model_y[clean_t].tolist()]
    ...
    #text_list = [' '.join(word_tokenize(x)[:MAX_SENT_LEN]) for x in [clean_alls]]
    ...
    #        seq = [t if t<MAX_VOCAB_SIZE else text_word_index['UNK'] for t in seq ]
    #    seq.append(text_word_index['EOS'])
    #    X[i] = seq
    #X = pad_sequences(X, padding='post', truncating='post', maxlen=MAX_SENT_LEN, value=text_word_index['PAD'])

    #text_word_index = {k: v for k, v in text_word_index.items() if v < MAX_VOCAB_SIZE} 
    ...
    #    try:
    #        embeddings_vector = N[word]
    #    except KeyError:
    #        embeddings_vector = None
    #    if embeddings_vector is not None:
    #        encoder_embeddings_matrix[i] = embeddings_vector

    #enc_input = Input(shape=(MAX_SENT_LEN, ), name='encoder_input')
